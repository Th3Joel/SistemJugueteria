...
func (BackupC) Generate(f *fiber.Ctx) error {
	...
	password := "Jo12el34//"
	database := "jugueteria"
	outputFile := "backups/" + time.Now().Format("2006-01-02_15:04:05") + ".sql"

	cmd := exec.Command("mysqldump", "-u"+user, "-p"+password, database)
...
func (BackupC) Restore(f *fiber.Ctx) error {
	...
	cmd := exec.Command("mysql", "-u"+user, "-p"+password, database)
